...
class DraftChangeSet(models.Model):
    ...
    class ChangeSetType(models.IntegerChoices):
        # We're adding DraftChangeSet as a later addition to the data model, so
        # we're going to force one migration where we pretend 
        MIGRATION_INIT = 0, _("Migration Initialization")

        # These are our normal operations.
        CREATE = 1, _("Create"),
        EDIT = 2, _("Edit"),
        DELETE = 3, _("Delete"),
